...
/**
 * Removes an object at a given index from the list.
 *
 * @param   plist Ptr to list obj
 * @param   index Index to be removed
 * @return  Return status
 */
PmReturn_t list_removeIndex(pPmObj_t plist, int16_t index);
...
/**
 * Implements slice storage.  Equivalent to the python expression plist[startIndex:endIndex]=piter
 * This is the actual implementation, and is faster if you can use it
 * @param plist Ptr to list obj
 * @param startIndex int which is the start index
 * @param endIndex int which is the end index
 * @param piter Ptr to something that's iterable, like a tuple or a list
 * @return Return status
 */
PmReturn_t list_storeSlice(pPmObj_t plist, int16_t startIndex, int16_t endIndex, pPmObj_t piter);

/**
 * Implements slice retrieval.  Equivalent to the python expression plist[startIndex:endIndex:skipBy].
 * Returns the result of the expression.
 * @param plist Ptr to list obj
 * @param startIndex int which is the start index
 * @param endIndex int which is the end index
 * @param step int by which to increment
 * @param rlist Ptr to new slice, which is a list
 * @return Return status
 */
PmReturn_t list_getSlice(pPmObj_t plist, int16_t startIndex, int16_t endIndex, int16_t step, pPmObj_t *rlist);
