 ...
 * ---
 *
 * 2008/01/19   Included locking structure
...
/** Reschedule bitmask */
#define RESCHEDULE_BITMASK (0x01)

/** Atomic bitmask */
#define ATOMIC_BITSHIFT 1
#define ATOMIC_BITMASK (0x01<<ATOMIC_BITSHIFT)

/***************************************************************
 * Macros
 **************************************************************/

#define VM_IS_RESCHEDULE() \
    (gVmGlobal.schedule & RESCHEDULE_BITMASK)

#define VM_IS_ATOMIC(vmGlobal) \
    (gVmGlobal.schedule & ATOMIC_BITMASK)

#define VM_SET_RESCHEDULE(val) \
    do \
    { \
        gVmGlobal.schedule &= (~RESCHEDULE_BITMASK); \
        gVmGlobal.schedule |= (val & RESCHEDULE_BITMASK); \
    } \
    while (0)

#define VM_SET_ATOMIC(val) \
    do \
    { \
        gVmGlobal.schedule &= (~(ATOMIC_BITMASK)); \
        gVmGlobal.schedule |= ((val<<ATOMIC_BITSHIFT) & (ATOMIC_BITMASK)); \
    } \
    while (0)
...
typedef struct PmVmGlobal_s
{
    ...
    /** Flag to trigger rescheduling or lock in atomic mode (prevents rescheduling) */
    uint8_t schedule;

    /** Dict for callbacks for interrupts and things */
    pPmDict_t callbacks;
} PmVmGlobal_t,
 *pPmVmGlobal_t;
